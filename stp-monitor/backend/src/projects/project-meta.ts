...
export const PROJECT_META: Record<string, ProjectMeta> = {
  'stp-erp': {
    ...
    recentWork: [
      'Rediseño funcional del ERP TERMINADO (25 tickets, 5 sprints, ERP-83 a ERP-107): matriz de permisos por rol×módulo (nuevo rol Finanza) en los 18 controladores; cambios de esquema en Inventario (estado de ubicación estructurado), Colaboradores (tipo + código correlativo) y Nómina (préstamos con descuento automático); reorganización de menús (Cotizaciones y Nómina a Finanzas, Archivos de cliente a página propia); filtros por cliente/proyecto en Cotizaciones/Pagos/Gastos/Tareas/Fichas, páginas de detalle nuevas para Gastos y Tareas, bug real corregido en Reportes (pestañas Proyecto/Cliente no mostraban su selector), nuevo reporte de Nómina; y para cerrar, motor de búsqueda global (clientes/proyectos/cotizaciones/tareas/archivos/colaboradores, respeta permisos por rol) y motor de notificaciones in-app (tarea asignada, cotización aprobada/rechazada, pago recibido) reemplazando los dos placeholders del header. Quedan solo 2 tickets deferred sin sprint (ERP-108: acotar Gastos de Manager a sus proyectos; ERP-109: decidir si `user` puede subir archivos).',
