...
export const PROJECT_META: Record<string, ProjectMeta> = {
  'stp-erp': {
    ...
    recentWork: [
      'Rediseño funcional en marcha: matriz de permisos por rol×módulo (nuevo rol Finanza) migrada a los 18 controladores restantes, verificada en vivo. Fase de cambios de esquema completa: Inventario con estado de ubicación estructurado (almacén/reparación/prestado/asignado a proyecto) en vez de texto libre; Colaboradores con tipo (Fijo/Contratista/Temporero) y código correlativo por tipo (F-001, C-001...); Nómina con sistema de préstamos a colaboradores, descontados automáticamente en cada pago generado. Fase de reorganización de menús completa: Cotizaciones y Nómina pasaron a Finanzas; Archivos de un cliente salió de su ficha a una página propia con botón dedicado (pestaña nueva); un bug reportado de Archivos de proyecto (mostraba el listado global) resultó ya no ser reproducible al verificarlo. Fase de filtros y bugs rápidos completa: filtro por cliente/proyecto en Cotizaciones, Pagos, Gastos, Tareas y Fichas de campo; páginas de detalle nuevas para Gastos y Tareas; tabla de Tareas ya no requiere scroll lateral; bug real corregido en Reportes (las pestañas Proyecto/Cliente sin selección no mostraban su selector); nuevo reporte de Nómina. Roadmap completo (25 tickets, 5 sprints) en STP Tickets, proyecto ERP — siguiente etapa: búsqueda y notificaciones.',
